...
def main(args):
    ...
    df_train_raw, df_val_raw, df_hold_raw = three_way_split(
        df_clean[[args.target_col]], train_ratio=0.8, val_ratio=0.19
    )

    # --- Добавление признаков ---
    logging.info("Добавление кастомных признаков (циклических и rolling) после разделения...")
    df_train = add_features(df_train_raw, args.target_col)
    df_val = add_features(df_val_raw, args.target_col)
    df_hold = add_features(df_hold_raw, args.target_col)
