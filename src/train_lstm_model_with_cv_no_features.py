...
LOGS_ROOT = Path("logs/lstm_univariate_cv")
...
def df_to_univariate_X_y(df: pd.DataFrame, window_size: int = 6):
    """Преобразует одномерный временной ряд в датасет с окнами для LSTM."""
    series = df.iloc[:, 0].to_numpy() # Берем первую (и единственную) колонку
    X = []
    y = []
    for i in range(len(series) - window_size):
        row = [[item] for item in series[i:i + window_size]]
        X.append(row)
...
def main(args):
    ...
    df_final = df_clean[[args.target_col]] # Используем только целевую колонку

    # --- Кросс-валидация ---
    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = Path(f"models/lstm_univariate_cv_{ts}")
    plots_dir = Path(f"plots/lstm_univariate_cv_{ts}")
    ...
    for fold_idx, (train_val_idx, test_idx) in enumerate(tscv.split(df_final)):
        fold_num = fold_idx + 1
        logging.info(f"==== ФОЛД {fold_num}/{args.n_splits} ====")
        
        # 1. Разделение на train+val и test для этого фолда
        df_train_val_fold = df_final.iloc[train_val_idx]
        ...
        X_train, y_train = df_to_univariate_X_y(df_train_fold, args.window_size)
        X_val, y_val = df_to_univariate_X_y(df_val_fold, args.window_size)
        X_test, y_test = df_to_univariate_X_y(df_test_fold, args.window_size)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Univariate LSTM Time Series Forecasting with Cross-Validation")
